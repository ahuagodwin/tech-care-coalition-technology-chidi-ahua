...
<div class="d-flex flex-column px-4">
    <div class="header d-flex justify-content-between">
       ...
       <div class="flex-grow-1 ms-3 text-end" v-if="!isAdmin">
    ...
    <img src="@/assets/img/auth/auth-top-right.svg" class="topImage" :class="{'is-admin': isAdmin }"/>
...
<script lang="ts" setup>
    import { getProjectRequests, deleteProject, getProjectRequestTeam } from '../../services'
    ...
    const props = defineProps<{isAdmin?:boolean; workspaceId?:string|number}>()
    ...
    const setProjectDetails = (projectDetails: any) => {
        const {
            ...
            execute: executeGetProjectTeam,
        } = getProjectRequestTeam(projectDetails.id);

        executeGetProjectTeam().then((res: any) => {
            projectTeamData.value = res
    ...
    onMounted(() => {
        executeGetProjectRequests(props.isAdmin && {url: `/api/admin/workspace/get_company_requests`, params:{company_id: props.workspaceId }}).then((res: any) => {
